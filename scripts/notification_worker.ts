...
async function sendSms(text: string) {
  if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_FROM) throw new Error('Twilio not configured');
...
async function processNotification(n: any) {
  ...
  try {
    ...
    console.error('Notification send failed', err.message || err);
    await supabase.from('notifications').update({ attempts: (n.attempts || 0) + 1, last_error: (err.message || err).toString() }).eq('id', n.id);
  }
}

async function fetchPending() {
  const { data, error } = await supabase.from('notifications').select('*').eq('status', 'pending').limit(20);
